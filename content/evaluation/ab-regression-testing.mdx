...
<Callout type="info">
  The test case comparer matches test cases from both test runs using the
  `input`s by default.
</Callout>
...
If you assign names to your test cases (for example, to incorporate external IDs from Snowflake or other data sources), you can match test cases between runs using these names instead of `input`s. Simply toggle the **Match by** option from **Input** to **Name** in the test case comparison view.

You can also watch the [video summary](/evaluation/ab-regression-testing#video-summary) above for more details.
...
This will show you all test cases where performance degraded (labelled in red) between runs. Watch the [video summary](/evaluation/ab-regression-testing#video-summary) above for more details.
...
Watch the [video summary](/evaluation/ab-regression-testing#video-summary) above for more details.
