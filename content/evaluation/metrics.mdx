...
These metrics are available both [via DeepEval](/evaluation/metrics#run-metrics-via-deepeval) and [on the cloud.](/evaluation/metrics#run-metrics-on-the-cloud)
...
All three custom metrics are available [via DeepEval](/evaluation/metrics#run-metrics-via-deepeval), only `GEval` is available and [on the cloud.](/evaluation/metrics#run-metrics-on-the-cloud)

> [!TIP]
>
> The [supported use cases page](/concepts/supported-use-cases) is a great resource to see how each custom metric can be used for specific use cases.
...
These metrics are mostly only available [via DeepEval](/evaluation/metrics#run-metrics-via-deepeval).
...
- [Ollama](https://docs.confident-ai.com/integrations/model-ollama)
- [Azure OpenAI](https://docs.confident-ai.com/integrations/model-azure-openai)
- [Anthropic](https://docs.confident-ai.com/integrations/model-anthropic)
- [Gemini](https://docs.confident-ai.com/integrations/model-gemini)
- [Vertex AI](https://docs.confident-ai.com/integrations/model-vertex-ai)
- [vLLM](https://docs.confident-ai.com/integrations/model-vllm)
- [LMStudio](https://docs.confident-ai.com/integrations/model-lmstudio)

Lastly, you can wrap your own LLM API in `deepeval`'s `DeepEvalBaseLLM` class to use **ANY** model of your choice. [Click here](https://docs.confident-ai.com/guides/guides-using-custom-llms) to learn how.
...
<Callout type="warning">
  ...
  [DeepEval's
  documentation](https://docs.confident-ai.com/docs/metrics-introduction) to
  learn how to use each of them.
</Callout>

#### Default

The default [`AnswerRelevancyMetric`](https://docs.confident-ai.com/docs/metrics-answer-relevancy) is simple and requires no additional configuration:
...
A [`GEval`](https://docs.confident-ai.com/docs/metrics-llm-evals) implementation of the same answer relevancy metric, with more tailor criteria that specifies that the `actual_output` has to be shorter than 3 sentences:
...
A [`DAGMetric`](https://docs.confident-ai.com/docs/metrics-dag) implementation is perfect for combining objective and subjective criteria. It is an LLM powered decision tree that can combine other metrics such as `AnswerRelevancyMetric` or `GEval`:
...
1. [Through an HTTPS `POST` request](/evaluation/running-evals#run-llm-evals-on-the-cloud) that sends over a list of test cases with the generated outputs from your LLM app, or
2. [On the platform directly](/evaluation/running-evals#trigger-evals-without-code), which will be triggered through the click of a button without the need of code
...
For instance, the `AnswerRelevancyMetric` [documentation page](https://docs.confident-ai.com/docs/metrics-answer-relevancy) provides a complete technical reference for that specific metric.
