...
<Callout type="info">
The `@observe` decorator automatically tracks [span](/llm-tracing/tracing-features/span-types) latency. Therefore, this guide will mainly focus on how to set up **cost tracking**.
</Callout>
...
You can either manually configure cost tracking or let Confident AI calculate your costs automatically from the [inputs and outputs](/llm-tracing/tracing-features/input-output) of your LLM span based on the provided `model`.
...
To set up automatic cost tracking, provide the `model` in the `@observe` decorator of your [LLM span](/llm-tracing/tracing-features/span-types#llm-span), and provide the `input` and `output` in your `LlmAttributes`.
...
To manually set up cost tracking, provide the `cost_per_input_token` and `cost_per_output_token` in the `@observe` decorator of your [LLM span](/llm-tracing/tracing-features/span-types#llm-span), and pass the input and output token counts via `LlmAttributes`.
