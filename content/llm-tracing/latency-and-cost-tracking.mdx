...
<Callout type="info">
The `@observe` decorator automatically tracks [span](/llm-tracing/tracing-features/attributes#span-attributes) latency. Therefore, this guide will mainly focus on how to set up **cost tracking**.
</Callout>
...
To set up automatic cost tracking, provide the `model` in the `@observe` decorator of your [LLM span](/llm-tracing/tracing-features/attributes#llm-attributes), and provide the `input` and `output` in your `LlmAttributes`.
