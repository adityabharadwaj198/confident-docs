...
1. Install the LiteLLM SDK:

```bash
pip install litellm
```

2. Use `litellm.success_callback` and `litellm.failure_callback` to trace your LLM calls:
...
1. Add **deepeval** to the `success_callback` and `failure_callback` in the `config.yaml` file.
...
4. Make a request to the proxy server:
