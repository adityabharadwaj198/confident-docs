...
instrument.get_dispatcher().add_span_handler(LlamaIndexSpanHandler())
```
...
<Callout>
As the agent is running, it will export **spans** to the `dispatcher`. The `dispatcher` will send the spans to the `LlamaIndexSpanHandler` which will collect them to create a **trace** and send it to the [Observatory](https://documentation.confident-ai.com/llm-tracing/introduction).
</Callout>
