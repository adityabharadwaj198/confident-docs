...
Confident AI's OpenTelemetry integration allows you to map attributes and events to Confident AI's [span type attributes](/llm-tracing/tracing-features/attributes). Below are the mappings for the `"llm"`, `"tool"`, `"agent"`, and `"retriever"` span types.
