...
To enable referenceless metrics to run in production, you will need to [create a metric collection](/llm-evaluation/metrics/create-on-the-cloud).
...
To recap, [referenceless metrics](/concepts/metrics#referenceless-metrics) section, referenceless metrics are a special type of metric that can evaluate your LLM's performance without requiring reference data (like `expected_output` or `expected_tools`).
</Callout>
...
<Tabs items={['Python', 'TypeScript']}>
  <Tabs.Tab>
```python filename="main.py" showLineNumbers copy {4, 13}
...
Running evals on traces are akin to running [end-to-end evals](/llm-evaluation/run-evals/end-to-end-evals), where you disregard the performance of individual spans within the trace and treat your application as a black-box.
...
```python filename="main.py" showLineNumbers copy {15, 23}
import openai
...
You **MUST** [set the `input`/`output`](/llm-tracing/tracing-features/input-output) of individual traces in a thread for multi-turn evaluation to work online. To recap, DeepEval uses the `input` of a trace as the `"user"` role content and the `output` of a trace as the `"assistant"` role content as turns in your thread.
