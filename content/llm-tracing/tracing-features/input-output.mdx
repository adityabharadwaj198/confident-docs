...
<Tabs items={['Python', 'JS/TypeScript']}>
  <Tabs.Tab>
    ```python filename="main.py" showLineNumbers {13}
    import openai
...
**HOWEVER**, if you are planning to [create a thread](/llm-tracing/tracing-features/threads) from the traces (for chatbots, conversations, etc. multi-turn use cases), it is highly recommended that you provide **strings** instead, where the `input` will represent the user input, and `output` representing the LLM generated output. You can also leave out any `input` or `output` for consecutive user/LLM behaviors. 

You will also need the `input` and `output` to run [online evaluations on a thread](/llm-tracing/online-evaluations#online-evals-for-threads), as these will be used as the turns for a conversational test case.
