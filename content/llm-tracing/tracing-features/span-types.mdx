...
This is set via the `type` argument in the `@observe` decorator. By classifying span types you can create more tailored UIs on Confident AI, and set more specific [`attribtues`](/llm-tracing/tracing-features/attributes) instead of using generalized [`metadata`](/llm-tracing/tracing-features/metadata).
...
- [Optional] `metrics` A list of strings specifying the names of the online metrics you wish to run upon tracing to Confident AI. Learn more about using online metrics in the [next section here.](/llm-tracing/tracing-features/online-evaluation)
