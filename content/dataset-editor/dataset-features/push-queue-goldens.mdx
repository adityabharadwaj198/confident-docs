...
DeepEval offers the `.push()` and `.queue()` method to upload goldens to Confident AI as an alternative to [importing goldens from CSV](/dataset-editor/dataset-features/import-from-csv).
...
<Callout>
By default, the `push` method marks goldens as **finalized**. Goldens that are finalized will be pulled by default via the [`.pull()` method](/dataset-editor/using-datasets#pull-a-dataset). To upload goldens as **unfinalized**, use the [queue method](#queue) instead.
</Callout>
