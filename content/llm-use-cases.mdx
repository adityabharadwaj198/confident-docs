...
- Complex systems (like code generation and autonomous agents) require both system metrics and **reference-based evaluation** against [golden `expected_output`s](/concepts/datasets#goldens), along with [tracing](/llm-observability/llm-tracing) for debugging
...
[Click here](/llm-evaluation/metrics) to learn how to implement and use these metrics for evaluation.

<Callout>
  The prompt in this case should be [logged as a
  hyperparameter](/llm-evaluation/running-llm-evals#logging-hyperparameters)
...
In this example, the original text is the `input` to a test case, while the summary is the `actual_output`. The prompts that gives the summarization instructions should be provided in the system prompt and [logged as hyperparameters](/llm-evaluation/running-llm-evals#logging-hyperparameters).
...
In this example, the user's travel requirements are the `input` to a test case, while the final itinerary and bookings are the `actual_output`. [Click here](/llm-evaluation/metrics) to learn how to implement and use these metrics for evaluation.

<Callout>
  For autonomous agents, [setting up tracing](/llm-observability/llm-tracing) is
...
In this example, the natural language query is the `input` to a test case, while the generated SQL is the `actual_output`, and the retrieved schema information is the `retrieval_context`. [Tracing](/llm-observability/llm-tracing) is also extremely helpful here to visualize the retrieved tables and SQL execution times.
...
In this example, the natural language requirements are the `input` to a test case, while the generated code is the `actual_output`. You'll definitely want [tracing](/llm-observability/llm-tracing) for this use case.
