...
It is extremely common for users to setup cron jobs or pre-deployment [unit-testing evaluations in CI/CD](/evaluation/unit-testing-in-cicd) to create a test run each time a change is made to your LLM application.
...
<Callout>
  ...
  section.](/evaluation/running-llm-evals)
</Callout>
...
You can also create test runs on the cloud by running evaluations directly on Confident AI, either using API requests or in a click of a button directly on the platform. [Click here](/evaluation/running-llm-evals#run-llm-evals-on-the-cloud) to learn how.
