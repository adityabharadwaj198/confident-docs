...
Hyperparameters in Confident AI are key-value pairs of type `Dict[str, Union[str, Prompt]]` that define what parameters are use in your LLM application. For example, a key-value pair of `"model": "gpt-4o"` allows you to see that this was the model you used for generating the `actual_output`s for a particular test run.
...
When a hyperparameter is a prompt, you can leverage Confident AI's prompt studio to versioning, edit, and management them in one centralized place. This allows Confident AI to tell you which prompt version performs best for your use case.
...
Prompts are text templates that guide LLM behavior by providing instructions, context, and structure for generating responses. A prompt template contains variables that are interpolated at runtime with dynamic values to create the final prompt sent to the LLM.
...
### Prompt Management in Confident AI
...
By clearly associating hyperparameters with specific components, you can more effectively optimize each part of your agentic system.
