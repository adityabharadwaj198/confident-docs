...
- Goldens contain a `custom_column_key_values` field, which allows you to use data from custom columns pairs you have [created on Confident AI's dataset editor](/dataset-editor/annotate-datasets#adding-custom-columns)
...
The `custom_column_key_values` field will be populated by your dataset's custom column values (if any) when you [pull a dataset](/dataset-editor/using-datasets#pull-a-dataset) from Confident AI.
...
At evaluation time, you will [pull your dataset from Confident AI](/dataset-editor/using-datasets), generate any required test case parameters such as the `actual_output`, before running evaluations on the set of newly created test cases. Here's what it looks like:
