...
An `LLMTestCase` is used to evaluate a single interaction with an LLM application. It requires at minimum:
...
> [!TIP]
>
> For more information on a `ToolCall`, more information can be found on [DeepEval's documentation.](https://docs.confident-ai.com/docs/evaluation-test-cases#tools-called)
...
A `ConversationalTestCase` represents a multi-turn conversation with an LLM. It requires:

- `turns` (`list[LLMTestCase]`): A list of `LLMTestCase`s representing each interaction in the conversation
...
A more detailed breakdown of each individual test case parameter can be found on [DeepEval's documentation](https://docs.confident-ai.com/docs/evaluation-test-cases).
