...
You'll learn how to use referenceless metrics exclusively in production in [this section](/observability/online-metrics), where we refer to them as online metrics on Confident AI.
...
Yes, [metrics can be ran on the cloud](/evaluation/metrics#metrics-on-the-cloud). However, we recommend starting with local evaluation first. Running metrics locally makes it easier to:
