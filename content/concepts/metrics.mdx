...
You'll learn how to use referenceless metrics exclusively in production in [this section](/llm-tracing/tracing-features/online-evaluation), where we refer to them as online metrics on Confident AI.
...
In fact, you'll find this pattern used when picking metrics for different use cases in the [use cases page.](/llm-use-cases) Here's a nice flow chart for the process:
...
Whichever metric that you find is a referenceless one in your final set of metrics, you can [enable it for online evaluation](/llm-tracing/tracing-features/online-evaluation) in production.
...
<Callout>
  ...
  section.](/llm-evaluation/metrics/create-locally)
</Callout>
...
Yes, [metrics can be ran on the cloud](/llm-evaluation/metrics/create-on-the-cloud). However, we recommend starting with local evaluation first. Running metrics locally makes it easier to:
