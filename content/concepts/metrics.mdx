...
You'll learn how to use referenceless metrics exclusively in production in [this section](/llm-observability/online-metrics), where we refer to them as online metrics on Confident AI.
...
Whichever metric that you find is a referenceless one in your final set of metrics, you can [enable it for online evaluation](/llm-observability/online-metrics) in production.
...
<Callout>
  A more comprehensive walkthrough can be found in the [evaluation metrics
...
Yes, [metrics can be ran on the cloud](/llm-evaluation/metrics#metrics-on-the-cloud). However, we recommend starting with local evaluation first. Running metrics locally makes it easier to:
