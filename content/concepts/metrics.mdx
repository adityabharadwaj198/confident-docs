...
- **DAG (Deep Acyclic Graph)**: A decision-tree based LLM-evaluated metric
...
- **Contextual Precision**: Measures the precision of the retrieved context
- **Contextual Recall**: Evaluates the recall of the retrieved context
...
- **Conversational G-Eval**: Evaluates the overall quality of a conversation
...
- **Conversation Relevancy**: Evaluates how relevant each response is to the ongoing conversation

These are particular useful for text-based chatbots or conversational voice agents.

### Other Metrics

- **Json Correctness**: Evaluates the correctness of JSON outputs
- **RAGAS**: Implements the RAGAS framework for RAG evaluation, no different than DeepEval's RAG metrics
- **Hallucination**: Measures the extent to which the LLM generates information not present in the context
- **Toxicity**: Evaluates the toxicity level of LLM outputs
- **Bias**: Measures the presence of bias in LLM outputs
- **Summarization**: Evaluates the quality of summarization tasks

## Choosing the Right Metrics

Different LLM applications require different metrics for effective evaluation, and the specific combination of metrics depend on your LLM use case and application architecture. As a general rule of thumb, **DON't**:

1. Use more than 5 metrics
...
Another tip is, usually the generic, system specific metrics (such as answer relevancy and contextual relevancy for RAG, tool correctness for agents) are easy to define, and for the remaining 1-2 metrics that are custom will be specific and tailored to use cases.

In fact, you'll find this pattern when picking metrics for different use cases in the [use cases page.](/supported-use-cases)
