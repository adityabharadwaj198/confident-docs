...
```python showLineNumbers copy {7, 10, 13, 19}
...
Or in [CI/CD pipelines if you're unit testing](/evaluation/unit-testing-in-cicd) using DeepEval's Pytest integration:
...
The `Prompt` class stores your prompt or prompt messages in either the `template` or `template_messages` class variable upon the `pull()` method, which contains the raw, pre-interpolated template(s).
