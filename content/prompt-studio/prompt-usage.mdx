...
## What About Prompt Messages?

You pull prompt messages the same way you would pull a single prompt, the difference is it is now stored in the `templates_messages` variable instead, and you can print it out to see it for yourself:

```python
...

print(prompt.templates_messages)
```

To be clear, here is an example of how you can use it for OpenAI's APIs:

```python showLineNumbers copy {14}
import openai
from deepeval.prompt import Prompt

prompt = Prompt(alias="your-alias-here")
prompt.pull()
prompt_messages = prompt.interpolate()
