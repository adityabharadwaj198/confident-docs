...
Consider this exact same LLM app/agentic workflow from the [previous section](/observability/llm-tracing):
...
To enable referenceless metrics to run in production, you will need to [create a metric collection](/evaluation/metrics#metrics-on-the-cloud), and press the **Enable for Monitoring** button for that metric collection.
...
<Callout>
  ...
  [`update_current_span_attributes()`](/observability/llm-tracing#set-runtime-attributes-for-default-spans)
