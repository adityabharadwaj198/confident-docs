...
- Allows you to [run online metrics](/observability/online-metrics) for spans and traces in production.
...
def retrieve_documents(query: str) -> List[str]:
    ...
    update_current_span_attributes(
        RetrieverAttributes(
            embeddingText=query, retrieval_context=fetched_documents
        )
    )
...
- [Optional] `metrics` A list of strings specifying the names of the online metrics you wish to run upon tracing to Confident AI. Learn more about using online metrics in the [next section here.](/observability/online-metrics)
...
<Callout>
  ...
  can [set the
  `retrieval_context`](/observability/llm-tracing#retriever-attributes) as the
  output of a retriever span, allowing you to maintain your existing function
  signatures without modification.
</Callout>

### LLM Attributes

LLM attributes track the `input`, `output`, and token usage of language model calls. You **MUST** set the attributes for an LLM span.
...
Retriever attributes track the `embedding_input` and `retrieved_context` in RAG pipelines. You **MUST** set the attributes for a retriever span.
...
def retrieve_documents(query: str) -> List[str]:
    ...
    update_current_span_attributes(
        RetrieverAttributes(embeddingText=query, retrieval_context=fetched_documents)
    )
    return fetched_documents
```
...
- `embeddingText`: The text of type `str` that needs to be embedded for vector search.
...
For a more detailed walkthrough, please refer to the [video summary above.](/observability/llm-tracing#code--video-summary)
...
Yes, you can learn how in the [next section.](/observability/online-metrics)
