...
Once you've set your metrics, you'll need to define what the test case parameters are by using the `update_current_span_test_case()` function.
...
def outer_function():
    ...
    def inner_function():
        # Here, update_current_span_test_case() will update the LLM span
        update_current_span_test_case(...)
```
...
3. Any calls to `update_current_span_test_case()` during `inner_function`'s execution will update the "inner" span, not the "outer" one
...
<Callout>
  ...
  `update_current_span_test_case()` function can be called in custom
  spans as well.
</Callout>
