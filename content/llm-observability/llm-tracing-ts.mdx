...
const fetchDocuments = ObserveRetriever({
  ...
  fn: async (query: string) => {
    ...
    const documents = ["Document about Paris", "Document about France"];
...
const researchTask = ObserveAgent({
  ...
  fn: async (request: string) => {
    ...
    const thought = "I need to check the weather before recommending activities.";
    const plan = "1. Check weather\n2. Suggest indoor/outdoor activities";
